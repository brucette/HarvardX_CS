...
int check_if_digits(string argument);
string encipher_text(string word, int key);
...
int main(int argc, string argv[])
{
    ...
    string results = encipher_text(plain, key);         // Encipher plaintext using function

}



// FUNCTION TO ENCIPHER PLAINTEXT
string encipher_text(string plain, int key)
{
    int n = strlen(plain);
    char c[n];
    ...
    printf("ciphertext: ");
    for (int i = 0; i < n; i++)                         // Iterate over c and print each character
    {
        printf("%c", c[i]);
    }
    printf("\n");
